...
CAN can(PB_8, PB_9, 1000000); // CAN Rx pin name, CAN Tx pin name,bus frequency
CANMessage rxMsg;
CANMessage txMsg;

// DRV832x pin
SPI drv_spi(PA_7, PA_6, PA_5); // SPI(mosi,miso,sclk)
DigitalOut drv_cs(PA_1);
...
void onMsgReceived()
{
    ...
    if ((rxMsg.id == CAN_ID))
    {
        controller.timeout = 0; // watch dog
        ...
        {
            unpack_cmd(rxMsg, &controller); // can command pack,16 bit position command, between -4*pi and 4*pi
...
void enter_torque_mode(void)
{
    ...
    // gpio.enable->write(1);
    controller.ovp_flag = 0; // over voltage flag
...
int main()
{
    ...
    */
    ...
    {
        E_OFFSET = 0.0f;
    } // Returns TRUE if x is a NaN.Encoder electrical offset
    if (isnan(M_OFFSET))
    {
        M_OFFSET = 0.0f;
    } // Encoder mechanical offset
    ...
    {
        I_MAX = 40;
    } // Torque limit (current limit = torque_limit/(kt*gear ratio))
